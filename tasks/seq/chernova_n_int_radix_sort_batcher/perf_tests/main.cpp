...
TEST(chernova_n_int_radix_sort_batcher_seq, test_pipeline_run) {
  const int k = 1000000;
  std::vector<int> in;
  ...
  // Create Task
  auto test_task = std::make_shared<chernova_n_int_radix_sort_batcher_seq::TestTaskSequential>(task_data);
...
TEST(chernova_n_int_radix_sort_batcher_seq, test_task_run) {
  const int k = 1000000;
  std::vector<int> in;
  GenerateRandomNumbers(k, in, 10000000);
  std::vector<int> out(k, 0);
