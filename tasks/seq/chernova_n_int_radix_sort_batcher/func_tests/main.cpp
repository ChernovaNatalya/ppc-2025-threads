...
TEST(chernova_n_int_radix_sort_batcher_seq, test_20_numbers) {
  ...
  chernova_n_int_radix_sort_batcher_seq::TestTaskSequential test_task(task_data);
  Execution(test_task);
  EXPECT_TRUE(CheckResult(out));
}

TEST(chernova_n_int_radix_sort_batcher_seq, test_20_big_numbers) {
...
TEST(chernova_n_int_radix_sort_batcher_seq, test_20_random_numbers) {
  const int k = 20;
  std::vector<int> in;
...
TEST(chernova_n_int_radix_sort_batcher_seq, test_50_random_numbers) {
  const int k = 50;
  std::vector<int> in;
  GenerateRandomNumbers(k, in, 1000);
  std::vector<int> out(k, 0);
...
TEST(chernova_n_int_radix_sort_batcher_seq, test_1000_numbers) {
  const int k = 1000;
  std::vector<int> in;
  GenerateRandomNumbers(k, in, 1000);
  std::vector<int> out(k, 0);
...
TEST(chernova_n_int_radix_sort_batcher_seq, test_1000_big_numbers) {
  const int k = 1000;
  std::vector<int> in;
